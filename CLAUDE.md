...
- **Do-not-feature and source links (v4):** `src/lib/donotfeature.ts` is a one-document KV registry that wins over any consent; `saveEdition` refuses a story or body naming someone on it and `create_draft` re-checks. Every featured story needs a `sourceUrl` or, for a story the founder told the editor directly, a short `sourceNote`; `create_draft` refuses without one.
...
- **Backlog (v4):** entries carry `submittedBy` (server-filled from the signed-in user). There are no roles: everyone who can sign in is an editor. Highlights from colleagues are reference material and never enter an issue automatically.
