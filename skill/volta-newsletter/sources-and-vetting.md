...
- `residencyLine` is the call to action above the Apply button, and it must make sense to someone who has never heard of the program. Write it as **two short parts, both taken from the live https://voltaeffect.com/ai-residency page**: (1) one sentence that names Volta's AI Residency and says what it is (the page's own description works: "Volta's AI Residency helps Atlantic Canadian AI startups validate market need, win paying customers, and build toward a credible growth path with coaching."), then (2) the how-to-apply fact, such as "Applications are open year-round." Do not use the second part alone. It always names the program, it may start with a short lead-in like "Want to build this?", and it stays under about 50 words. Fetch that exact address; do not search for it. The page said this when last read; read it live and do not rely on this note. If you truly cannot open it, use the plain line from `brand-and-html.md` ("Got an idea for an AI startup? See how far you can take it with Volta's AI Residency.") and tell the editor. **Not opening the page never means skipping the idea.** Never write a deadline from memory.
