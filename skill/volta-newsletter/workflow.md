...
1. Run steps 1 to 9 of the weekly flow, then stop. A scheduled run continues an `in_progress` edition, and starts a new one if the last is `drafted` or none exists; it never touches an edition that has a Mailchimp draft (re-pushing needs `overwrite` and could discard the editor's edits).
2. In step 3, also look at the network startup list. It is a separate, external resource kept by the editor or whoever maintains it, **not the backlog** (the backlog is the editor's own notes). Ask for it if you were not given one. Search the web for recent news and jobs pages (each startup's own careers page only) for a small, different handful each run; `backlog_list` shows who was noted recently. Treat every result as a lead: it becomes a **new** backlog entry (`backlog_add`) or a note on an existing one (`backlog_update` with `appendNote`), never a line in the body.
