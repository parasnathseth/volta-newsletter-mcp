...
2. **Scan the community for news** (a scheduled run has time for this; keep it steady, not exhaustive):
   - **Regional searches.** Run three short web searches for news about Atlantic Canadian and Halifax AI startups in the last two weeks (for example new funding, launches, awards, partnerships, hiring), including local outlets. These find companies that are on no list.
   - **The backlog, a batch at a time.** Call `backlog_list` with `dueBy` set to today (soonest first) and take **up to 15** entries that are due; an entry with no revisit date counts as due. For each, look at the startup's own news page and do one quick news search. Then call `backlog_update` once for it: `appendNote` with a dated line (write "nothing new" if there is nothing) and a `revisitDate` 21 days ahead, so the rest of a long list come up on later runs, oldest first. Do not try to check everything in one run. A story stays fresh for 45 days, so one found a week or two late is still usable.
   - **If the editor has given you a list of startups** (a separate, external resource, **not the backlog**), add them to the backlog first with `backlog_add` (up to about 25 an hour; the rest on later runs), each with today as `revisitDate`, then work through them as above. If you were not given a list, do not mention it, stop or ask.
   - Treat every result as a **lead**: something newsworthy from the last 45 days becomes a candidate for the founder story (an item you vet), never a line in the body by itself. Say plainly in your summary how many startups you checked and that the rest are queued.
