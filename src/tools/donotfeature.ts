...
import { z } from 'zod';
import { addDoNotFeature, DoNotFeatureError, getDoNotFeature, removeDoNotFeature } from '../lib/donotfeature.ts';
...
export function registerDoNotFeatureTools(server: McpServer, env: Env, userEmail: () => string | undefined): void {
  ...
  server.registerTool(
    'do_not_feature_add',
    {
      ...
      inputSchema: {
        name: z.string().describe('The person or company that asked not to be featured, exactly as the editor said it (for example "Tidewater Maps").'),
        note: z.string().optional().describe('Who asked, when and how, in the editor\'s words (max 500 characters). Shown to the editor whenever this name blocks a save.'),
      },
    },
    async (args) => {
      try {
  ...
  server.registerTool(
    'do_not_feature_remove',
    {
      ...
      inputSchema: { name: z.string().describe('The name to remove, as shown by do_not_feature_list.') },
    },
    async ({ name }) => {
      try {
        await checkRateLimit(env, 'do_not_feature_remove', user());
        const removed = await removeDoNotFeature(env, name, user());
        logEvent('tool.do_not_feature_remove', { user: user(), id: removed.id });
